...
if (isset($_POST['data_ajax_adresa_client'])) {
    if (is_array($_POST['data_ajax_adresa_client']) and count($_POST['data_ajax_adresa_client']) != 0) {
        $search = implode("','", $_POST['data_ajax_adresa_client']);
        $sql .= " adresa_client IN ('{$search}') AND ";
    }
}
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $linkName = '<a target="_blank" href="/crm/pages/client-page.php?slug=' . $slug . '">' . htmlspecialchars($row['last_name_first_name']) . '</a>';

    $sub_array[] = $row['id'];
    $sub_array[] = $linkName;
    $sub_array[] = htmlspecialchars($row['adresa_client']);
    $sub_array[] = htmlspecialchars($row['phone_number']);
    $sub_array[] = htmlspecialchars($row['user_email_field']);
    $sub_array[] = htmlspecialchars($row['nr_identitate']);
